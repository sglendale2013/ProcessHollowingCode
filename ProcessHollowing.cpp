...
void CreateHollowedProcess(char* pDestCmdLine, char* pSourceFile)
{
	...
	//A user-defined method for ReadProcessMemory()
	...
	//debug printing
	printf("size of the replacing file %d\n", dwSize);
	//----
	...
	if (!pRemoteImage)
	{
		...
		printf("VirtualAllocEx call failed.\r\n");
		//1) Close previous handle; if the functions succeeds, the return value is nonzero.
		if (TerminateProcess(pProcessInfo->hProcess, 0) != 0) {
			printf("Terminated successfully. %d\r\n", instance_count);
		}
			//system("pause");
			//2) Call CreateHollowedProcess(char* pDestCmdLine, char* pSourceFile); 
		if (CloseHandle(hFile) != 0) {
			if (instance_count < 8)
			{
				...
				printf("Hollowing failed 8 times.\r\n");
				return;
			}
		}
